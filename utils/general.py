...
def box_dious(box1, box2):

    dist = F.pairwise_distance(box1[:,:2],box2[:,:2],p=2)+ \
           F.pairwise_distance(box1[:,2:],box2[:,2:],p=2) + \
           F.pairwise_distance(box1[:,::3],box2[:,::3],p=2) + \
           F.pairwise_distance(torch.stack((box1[:,2], box1[:,1]), dim=1),torch.stack((box2[:,2],box2[:,1]), dim=1),p=2)

    dist2=F.pairwise_distance(torch.min(box1[:,:2],box2[:,:2]),torch.max(box1[:,2:],box2[:,2:]), p=2)
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            lbox += ((1.0 - iou) + box_dious(pbox, tbox[i])).mean()  # iou loss
            # diou= box_dious(pbox, tbox[i])
            # cdiou=iou+0.001*(1-diou)
            # lbox +=(1.0 - iou)+diou
