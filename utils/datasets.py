...
class LoadImages_files:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        xx=files[0].split('.')[-1].lower()

        images = [x for x in files if os.path.splitext(x)[-1] in img_formats]
        videos = [x for x in files if os.path.splitext(x)[-1] in vid_formats]
